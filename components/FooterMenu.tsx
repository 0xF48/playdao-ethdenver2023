...
function ScanIcon({ sel }: any) {
	...
	return <svg onMouseEnter={() => { setColor('rgb(34 197 94)') }} onMouseLeave={() => { setColor('white') }} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke={color} class="w-6 h-6">
...
export default function () {
	...
	return <div className="w-full p-4 flex items-center content-center justify-center fixed bottom-0 left-0 pointer-events-none">
		<div className='scale-100 hover:scale-115 transition-transform transform-gpu pointer-events-auto scale-1 cursor-pointer border-2	 border-base-800 w-auto h-16 bg-black rounded-2xl text-white flex flex-row relative px-0'>
			...
			<div className={"transition-all transform-gpu ease-out absolute -bottom-1 w-8 h-2 rounded-full bg-green-500 " + cn({
				"animate-position": true,
				"left-5": router.pathname == '/',
				"left-1/2 -translate-x-1/2": router.pathname == '/quests',
				'right-5': router.pathname == '/badges'
			})} />
		</div>
	</div>
}